...
def truncate_to_width(text, font_name, font_size, max_width):
    ...
    clipped = text
    while clipped and (pdfmetrics.stringWidth(clipped, font_name, font_size) + ellipsis_width) > max_width:
        clipped = clipped[:-1]
    return f"{clipped.rstrip()}{ellipsis}"


def draw_fitted_text(c, text, x, y, max_width, font_name, font_size, min_size=7.0, align="left"):
    content = str(text or "")
    size = float(font_size)
    min_size = max(5.0, float(min_size))

    while size > min_size and pdfmetrics.stringWidth(content, font_name, size) > max_width:
        size -= 0.25

    if pdfmetrics.stringWidth(content, font_name, size) > max_width:
        content = truncate_to_width(content, font_name, size, max_width)
...
def draw_label_value_line(c, x, y, label, value, max_width, fonts, font_size=10, min_size=7.0):
    label_text = f"{label}: "
    c.setFont(fonts["bold"], font_size)
    c.drawString(x, y, label_text)

    label_width = pdfmetrics.stringWidth(label_text, fonts["bold"], font_size)
    value_x = x + label_width
    value_width = max(10, max_width - label_width)
    draw_fitted_text(
        c,
        value,
        value_x,
...
def table_text_density(row_count, base_font):
    if row_count <= 12:
        return float(base_font), 4.0
    if row_count <= 18:
        return max(7.8, float(base_font) - 0.5), 3.4
    if row_count <= 24:
        return max(7.2, float(base_font) - 1.0), 2.9
    return max(6.7, float(base_font) - 1.4), 2.4
...
def build_subject_rows(student):
    ...
    for subject, details in sorted(student["subjects"].items()):
        subject_label = subject if len(subject) <= 34 else f"{subject[:31].rstrip()}..."
        rows.append(
            [
                subject_label,
...
def draw_footer(c, width, fonts, school):
    ...
    draw_fitted_text(
        c,
        "Generated by Report Card System",
        right_x,
        18 * mm,
...
def draw_summary_lines(c, x, start_y, max_width, fonts, student):
    c.setFillColor(colors.HexColor("#1C1D21"))
    draw_label_value_line(
        c,
        x,
    ...
    draw_label_value_line(
        ...
        start_y - 12 * mm,
        "Overall Position",
        ordinal(student["overall_position"]),
        max_width=max_width,
        fonts=fonts,
        font_size=9.8,
        min_size=7.5,
    ...
    draw_label_value_line(
        ...
        start_y - 18 * mm,
        "Remark",
        remark_from_average(student["overall_average"]),
        max_width=max_width,
        fonts=fonts,
        font_size=9.8,
        min_size=7.5,
...
def render_classic_template(c, width, height, fonts, school, student, rows):
    ...
    logo_reserve = 52 * mm if school["logo_style"] != "watermark" else 0
    header_width = max(70 * mm, content_width - logo_reserve)
    ...
    draw_label_value_line(
        ...
        height - 92 * mm,
        "Template / Theme",
        f"{TEMPLATE_LABELS[school['template_id']]} | {school['theme_primary']}",
        max_width=content_width,
        fonts=fonts,
        font_size=9.8,
        min_size=7.4,
    ...
    table_font_size, table_pad = table_text_density(len(rows), 8.8)
    ...
    draw_summary_lines(c, 18 * mm, summary_y, content_width, fonts, student)
    draw_footer(c, width, fonts, school)


def render_modern_template(c, width, height, fonts, school, student, rows):
    primary = colors.HexColor(school["theme_primary"])
    secondary = colors.HexColor(school["theme_secondary"])
    ...
    c.setFillColor(colors.white)
    draw_fitted_text(c, "Theme", 10 * mm, 50 * mm, 38 * mm, fonts["bold"], 9.0, min_size=7)
    draw_fitted_text(c, school["theme_primary"], 10 * mm, 44 * mm, 38 * mm, fonts["regular"], 8.7, min_size=6.8)
    draw_fitted_text(
        c,
        TEMPLATE_LABELS[school["template_id"]],
        10 * mm,
        38 * mm,
        38 * mm,
        fonts["regular"],
    ...
    main_width = width - main_x - 14 * mm
    c.setFillColor(colors.HexColor("#151A28"))
    draw_fitted_text(c, "Report Card", main_x, height - 20 * mm, main_width, fonts["bold"], 18, min_size=12)
    draw_fitted_text(
        c,
    ...
    c.roundRect(main_x, height - 44 * mm, main_width, 13 * mm, 3 * mm, stroke=0, fill=1)
    c.setFillColor(colors.HexColor("#0E1320"))
    position_text = f"Position: {ordinal(student['overall_position'])}"
    position_width = min(44 * mm, main_width * 0.45)
    draw_fitted_text(
        c,
        position_text,
        width - 16 * mm,
    ...
    draw_label_value_line(
        ...
        "Name",
        student["student_name"],
        max_width=(main_width - position_width - 4 * mm),
        fonts=fonts,
        font_size=9.2,
        min_size=7,
    )
    ...
    table_font_size, table_pad = table_text_density(len(rows), 8.2)
    ...
    draw_summary_lines(c, main_x, summary_y, main_width, fonts, student)
    draw_footer(c, width, fonts, school)


def render_minimal_template(c, width, height, fonts, school, student, rows):
    primary = colors.HexColor(school["theme_primary"])
    secondary = colors.HexColor(school["theme_secondary"])
    ...
    left_info_width = content_width * 0.58
    right_info_width = content_width * 0.36
    draw_label_value_line(
        c,
        content_left,
        height - 38 * mm,
    ...
    draw_fitted_text(
        c,
        f"Template: {TEMPLATE_LABELS[school['template_id']]}",
        content_right,
        height - 38 * mm,
        right_info_width,
        fonts["regular"],
    ...
    draw_fitted_text(
        c,
        f"Position: {ordinal(student['overall_position'])}",
        content_right,
        height - 50 * mm,
        right_info_width,
        fonts["bold"],
    ...
    draw_summary_lines(c, 20 * mm, summary_y, width - 40 * mm, fonts, student)
    draw_footer(c, width, fonts, school)
...
def render_report_card(pdf_path: Path, school, student):
    ...
    if template_id == "modern":
        render_modern_template(c, width, height, fonts, school, student, rows)
    elif template_id == "minimal":
        render_minimal_template(c, width, height, fonts, school, student, rows)
    else:
        render_classic_template(c, width, height, fonts, school, student, rows)
